...
The implementation of `class::knn()` is in some ways special and different from the implementation of other algorithms. One of these pecularities is that `class::knn()` does not return a fitted model but instead returns the predicted values directly. Depending on the value of the argument `prob`, these results also include the probability values of the predicted classes.

### The `cross_validation` Method

Inside of this function, a k-fold cross validation should be performed for one specific hyperparameter setting. It must take the arguments `x`, `y`, `params` (a list of hyperparameters), `fold_list` (to define the cross-validation folds), `ncores`, and `seed`.
...
results <- knn_optimization$execute(k = 3)
head(results)
```
...
results <- knn_optimization$execute()
head(results)
```

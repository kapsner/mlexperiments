...
* Hyperparameter tuning: with the R6 class `mlexperiments::MLTuneParameters`, to optimize the hyperparameters in a k-fold cross-validation with one of the two strategies
  + Grid search
  + Bayesian optimization (using the [`rBayesianOptimization`](https://github.com/yanyachen/rBayesianOptimization) R package)
...
knn_bsF <- function(...) { # nolint
  ...
  bayes_opt_knn <- knn_optimization(
    ...
    ncores = 1L, # currently, this cannot be parallelized
    seed = seed
  )
...
More details on the package [`rBayesianOptimization`](https://cran.r-project.org/package=rBayesianOptimization) and on how to define the Bayesian scoring function can be found in its [package vignette](https://cran.r-project.org/package=rBayesianOptimization/vignettes/tuningHyperparameters.html#practical-example).
...
| `private$fun_bayesian_scoring_function` | function | A function that is defined according to the requirements of the [`rBayesianOptimization`](https://cran.r-project.org/package=rBayesianOptimization) R package. It must return a named list that contains the optimization metric as the item `Score`. |
...
Please note that `metric_optimization_higher_better` is defaulted to `FALSE` here when initializing the super-class. This is because of choosing the mean misclassification error as the optimization metric (`FUN <- metric("MMCE")`) when defining the `cross_validation`-function above.
